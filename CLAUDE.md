...
- **Nom du site** : Meilleure Chemise
- **Description (FR)** : Le guide independant pour choisir, porter et entretenir sa chemise homme et femme : matieres, coupes, occasions et entretien.
- **Description (EN)** : The independent guide to choosing, wearing and caring for men's and women's shirts: fabrics, cuts, occasions and care.
- **URL** : https://meilleure-chemise.com (domaine acquis le 2026-09-16, DNS/GitHub Pages a connecter)
- **Couleurs** : primary `#1B2A41` (navy), background `#FAF6EF` (creme), accent `#B08D57` (bronze)
- **Polices** : titres Playfair Display, corps Lora, UI Inter
- **Langue principale** : fr (la version EN en sous-dossier `/en/` est TOUJOURS active)
- **Categories (FR ↔ EN)** : "Matieres et tissus / Fabrics and Materials", "Coupes et style / Cuts and Styles", "Occasions et dress code / Occasions and Dress Code", "Entretien et durabilite / Care and Durability"
- **Auteur principal du site** : magalie-ergoz (mode/beaute, meilleur match thematique chemise)
- **Client dedie** : Café Coton, exclusif (voir `Clients/Café Coton/REFS.md`). Cadence : 4 articles/semaine max (limite reseau standard, pas d'exception malgre la demande initiale de 1/jour).
- **Particularite technique multilingue** : le contenu anglais vit dans `content-en/` (PAS `content/en/`), avec `contentDir` explicite par langue dans `hugo.toml`. Ne jamais deplacer `content-en/` sous `content/en/` : cela reintroduit un bug de fuite de taxonomies (categories/tags anglais publies par erreur sous la racine FR). Voir section "Piege multilingue" plus bas.
...
## Piege multilingue : ne jamais imbriquer content-en/ sous content/

**Symptome observe** (2026-09-16) : les pages de taxonomie (categories, tags) en anglais (ex: `/categories/fabrics-and-materials/`) etaient publiees SOUS LA RACINE FRANCAISE (sans prefixe `/en/`), avec `lang="fr-FR"` dans le HTML alors que le contenu etait en anglais. Consequence : URLs dupliquees, hreflang casse, contenu mixte indexable par Google. Le probleme touchait aussi les compteurs de la page d'accueil ("Nos categories" affichait 8 termes au lieu de 4) et gonflait le nombre de pages FR construites (126 au lieu d'une cinquantaine).

**Cause etablie** : le contenu anglais vivait dans `content/en/`, un sous-dossier DE `content/` (le `contentDir` de la langue par defaut fr). Meme avec `contentDir` explicite par langue dans `hugo.toml` (`fr` → `content`, `en` → `content/en`), Hugo scanne le `contentDir` du fr de facon recursive et retombe sur les fichiers anglais nested dedans, les traitant comme du contenu francais additionnel (en plus de les traiter correctement comme anglais via son propre `contentDir`). Resultat : chaque fichier anglais est construit DEUX FOIS, une fois correctement sous `/en/`, une fois par erreur sous la racine fr.

**Correctif applique** : deplacer tout le contenu anglais dans un dossier FRERE, hors de `content/` : `content-en/` (pas `content/en/`). Configuration finale dans `hugo.toml` :
```toml
[languages.fr]
  contentDir = "content"
[languages.en]
...
```
Avec ce decoupage en dossiers non-imbriques, chaque langue est scannee une seule fois. Statut : **resolu**.
